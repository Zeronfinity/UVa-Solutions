...
double vec_magni(vec v)
{
    return sqrt(v.x * v.x + v.y * v.y);
}
double distToLine(pointd p, pointd a, pointd b, pointd &c)   //also creates closest point c
{
    vec ap = toVec(a, p), ab = toVec(a, b);
    double u = dot(ap, ab) / vec_magni(ab);
    c = translate(a, scale(ab, u));
    return dist(p, c);
}

double distToLineSegment(pointd p, pointd a, pointd b, pointd &c)   //also creates closest point c
{
    vec ap = toVec(a, p), ab = toVec(a, b);
    double u = dot(ap, ab) / vec_magni(ab);
    if (u < 0.0)
